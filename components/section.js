...
export const Section = {
  ...
  view(vnode) {
    ...
    return m('div', { key: section.name, class: 'flex flex-col gap-2 pb-3' }, [
      m(
        'div',
        {
      ...
      !vnode.state.open
        ? m('div', { class: 'text-sm opacity-50 italic' }, section.description)
        : list
            .filter((item) => item.section === section.name)
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((item) =>
              m(
                listState !== config.listState.draft ? ItemNameReady : ItemName,
                {
                  key: item.name,
                  item,
                  listId,
